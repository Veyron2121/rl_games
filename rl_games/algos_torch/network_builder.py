...
class CNetworkCont(NetworkBuilder):
    def __init__(self, output_dim, **mlp_args):
        ...
        self.net = nn.Sequential(*list(self.net.children()),
                                    ...
                                    self.activations_factory.create('sigmoid'))
...
class CLearningTD3Builder(NetworkBuilder):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            critic_mlp_args = {
                'input_size' : obs_dim + goal_dim + action_dim + 1, 
                'units' : self.units, 
                'activation' : self.activation, 
                'norm_func_name' : self.normalization,
                'dense_func' : torch.nn.Linear,
            ...
            self.critic1 = self._build_critic(1, **critic_mlp_args)
            self.critic2 = self._build_critic(1, **critic_mlp_args)

            self.target_actor = self._build_actor(action_dim, **actor_mlp_args)
            self.target_critic1 = self._build_critic(1, **critic_mlp_args)
            self.target_critic2 = self._build_critic(1, **critic_mlp_args)
        ...
        def _build_critic(self, output_dim, **mlp_args):
            return CNetworkCont(output_dim, **mlp_args)

        def _build_actor(self, output_dim, **mlp_args):
            return ActorNetwork(output_dim, **mlp_args)
        ...
        def updateTargets(self, target, original, tau=0.05):
        ...
        def update_targets(self, tau=0.05):
            self.updateTargets(self.target_critic1, self.critic1, tau)
            self.updateTargets(self.target_critic2, self.critic2, tau)
            self.updateTargets(self.target_actor, self.actor, tau)
